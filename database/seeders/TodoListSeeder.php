...
//        $todoList->todos()->save(Todo::factory()->make());
        ...
        $todoList->each(function ($list) use($tags){
            Todo::factory(5)->create(['todo_list_id' => $list->id])
                ->each(function ($todo) use ($list,$tags) {
                $todo->tags()->attach($tags->random(rand(1, 3))
                    ->pluck('id')->toArray());
                $list->todos()->save($todo);
            });
        });

//        TodoList::factory(5)->create()->each(function ($todoList) {
//            Todo::factory(15)->create(['todo_list_id'=>$todoList->id])
//                ->each(function($todo) use($todoList) {
